...
class BacktestEngine:
    ...
    def _reset_run_state(self) -> None:
        # Keep user-seeded holdings/cash intact; only clear per-run artifacts.
...
def _extract_series_for_symbol(market_data: dict, sym: str, column: str, cfg: UltimateConfig) -> pd.Series:
    ...
    if column == "volume":
        return row.get("Volume", pd.Series(0.0, index=row.index)).fillna(0.0)
...
def run_backtest(
    ...
    if all_target_dates.empty:
        # Short windows can miss the configured cadence entirely (e.g., W-FRI
        # over two non-Friday dates). Fall back to a terminal rebalance anchor.
        all_target_dates = pd.DatetimeIndex([end_ts])
