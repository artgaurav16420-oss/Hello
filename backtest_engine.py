...
def run_backtest(
    ...
    if clip_start_iloc < 0:
        clip_start_iloc = 0
    # Skip the first row only when it is all-NaN (the artefact of pct_change on
    # unclipped data).  In the custom-universe branch _clip() already starts
    # returns at warmup_start, so the first row is valid; unconditionally
    # advancing by 1 there would cause a double-advance and silently drop a
    # real bar.
